...
// Constructor for ProcessingEngine class that accepts the number of threads and affinity flag
...
void ProcessingEngine::indexFiles(const std::string& path) {
    ...
    // Initialize the character dictionary and delimiters
    char charDict[256];      // Character dictionary for tokenization
    char delimiters[256];    // Delimiter array for tokenization
    int delimiterCount = 0;  // Counter for the number of delimiters
    initializeCharDict(charDict, delimiters, delimiterCount);  // Initialize the character dictionary and delimiters

    std::mutex tokenMutex, bytesMutex;  // Mutexes for thread-safe operations

    std::vector<double> tokenizationTimes(numThreads, 0.0);  // Vector to store tokenization time for each thread
    ...
    for (int i = 0; i < numThreads; ++i) {
        processingThreads.emplace_back(
            ...
            std::ref(bytesMutex),
            charDict,
            delimiters,         // Pass delimiters array
            delimiterCount,     // Pass delimiter count
            std::ref(path),
            std::ref(path),     // Assuming resultPath is same as path or adjust accordingly
            std::ref(totalBytes),
            std::ref(totalTokens),
    ...
    double totalTime = totalDuration.count();  // Total time in seconds

    int longestThreadId = 0;  // ID of the thread with the longest tokenization time
    double longestTime = 0.0;  // Longest tokenization time
    for (int i = 0; i < numThreads; ++i) {
...
// Tokenization function using strtok_r and std::vector<char*>
std::vector<char*> ProcessingEngine::tokenize(char* buffer, char* delimiters) {
...
void ProcessingEngine::processFile(int thread_id,
    ...
    while (true) {
        ...
        //             fileData = std::move(fileBuffersPerNode[i].front());
        ...
        std::vector<char*> tokens = tokenize(buffer, delimiters);
    ...
    // No need to delete delimiters array here as it's managed in indexFiles
}

// Initialize the character dictionary and delimiters
void ProcessingEngine::initializeCharDict(char charDict[256], char* delimiters, int& delimiterCount) {
    // Initialize all characters as non-delimiters
    for (int i = 0; i < 256; i++) {
        charDict[i] = ~0;  // Set all characters as non-delimiters initially
    }
    int index = 0;
    for (int i = 1; i < 256; i++) {
        if (!isalnum(static_cast<unsigned char>(i))) {  // Set delimiters (non-alphanumeric characters)
            charDict[i] = 0;
            delimiters[index++] = static_cast<char>(i);  // Add the delimiter to the delimiters array
        }
    }
    charDict[0] = 0;  // Optional: Depending on your use case
    delimiters[index] = '\0';  // Null-terminate the delimiters array
    delimiterCount = index;  // Set the delimiter count
}
