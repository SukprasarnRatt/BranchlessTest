...
ProcessingEngine::ProcessingEngine(int numThreads, int affinityFlag) {
    ...
    this->affinityFlag = affinityFlag;  // Initialize the affinityFlag
}
...
void ProcessingEngine::loadFilesOnNode(int thread_id,
    ...
    for (const auto& [filePath, fileSize] : files) {
        ...
        // Allocate a string to hold the file content
        std::string buffer(fileSize, '\0');  // Initialize string with fileSize null characters

        // Read the file content into the buffer
        ssize_t bytesRead = read(fd, &buffer[0], fileSize);
        if (bytesRead == static_cast<ssize_t>(fileSize)) {
            buffer[fileSize] = '\0';  // Null-terminate the buffer (optional since std::string manages its own null terminator)
...

            {
                ...
                fileBuffer.push({filePath, buffer, fileSize});
            }
        } else {
            // If reading failed, print an error and handle accordingly
            std::lock_guard<std::mutex> guard(cout_mutex);
...
void ProcessingEngine::indexFiles(const std::string& path) {
    ...
    // Determine the thread with the longest tokenization time
...
std::vector<std::string> ProcessingEngine::tokenize(const std::string& content, const std::regex& tokenPattern) {
    std::vector<std::string> tokens;
    std::sregex_token_iterator iter(content.begin(), content.end(), tokenPattern);
...
void ProcessingEngine::processFile(int thread_id,
    ...
    while (true) {
        ...
        std::string buffer = fileData.content; // Get the buffer content as a string

        // Define a regex pattern for tokenization (adjust the pattern as needed)
        std::regex tokenPattern("[a-zA-Z0-9]+"); // Example: matches words consisting of alphanumeric characters

        // Tokenize the buffer directly using regex
        auto tokenStart = std::chrono::high_resolution_clock::now();

        // Call the tokenize function
        std::vector<std::string> tokens = tokenize(buffer, tokenPattern);
        ...
        // No need to manually delete memory since we're using std::string
        // The string will be destructed automatically when it goes out of scope
